module Employees
  class ItemsController < BaseController
    ...
    def new
      # @vendor positionné par Employees::BaseController#set_employee_for_view
      Rails.logger.info("📌 [Employees::ItemsController#new] current_shop: #{@current_shop&.id}, vendor: #{@vendor&.id}")
    ...
    def create
      # @vendor positionné par Employees::BaseController#set_employee_for_view
      return redirect_to url_with_shop(employees_items_path, @current_shop), alert: t("vendors.items.shop_required") unless @current_shop
    ...
    def purge_image
      ...
      respond_to do |format|
        format.turbo_stream
        format.html { redirect_to url_with_shop(edit_employees_item_path(@item), @current_shop), notice: "Image supprimée avec succès" }
      end
    rescue ActiveRecord::RecordNotFound
      respond_to do |format|
        format.turbo_stream { head :not_found }
        format.html { redirect_to url_with_shop(edit_employees_item_path(@item), @current_shop), alert: "Image non trouvée" }
      end
    end
    ...
    def generate_variants
      ...
      Rails.logger.info("🎯 [Employees::ItemsController] Génération de #{created_count} variantes - item_id: #{@item.id}")
    ...
    def generate_ai_preview
      # @vendor positionné par Employees::BaseController#set_employee_for_view
      return redirect_to url_with_shop(employees_items_path, @current_shop), alert: t("vendors.items.shop_required") unless @current_shop
      return redirect_to url_with_shop(employees_items_path, @current_shop), alert: t("vendors.items.ai_preview_unavailable") unless @current_shop.capabilities.ai_title_description_enabled?
    ...
    def retry_ai_enrichment
      ...
      redirect_to url_with_shop(employees_item_path(@item), @current_shop),
    ...
    def bulk_upload
      ...
      unless file.respond_to?(:read)
        redirect_to url_with_shop(employees_items_path, @current_shop), alert: t("vendors.items.bulk_upload.flash.no_file")
        return
      end
      ...
      redirect_to url_with_shop(employees_items_path, @current_shop)
    end
    ...
    def authorize_vendor_item!
      redirect_to url_with_shop(employees_items_path, @current_shop), alert: t("common.unauthorized") unless @item
    end
    ...
    def redirect_with_return_to(notice)
      if params[:return_to] == "manage"
        redirect_to url_with_shop(manage_employees_item_path(@item), @current_shop), notice: notice
      else
        redirect_to url_with_shop(employees_item_path(@item), @current_shop), notice: notice
      end
    end
    ...
    def handle_successful_creation(item)
      if item.variants.any?
        ...
        redirect_to url_with_shop(employees_item_path(item), @current_shop), notice: message
      else
        Rails.logger.warn("⚠️ [Employees::ItemsController] Produit créé SANS variante - item_id: #{item.id}")
        redirect_to url_with_shop(edit_employees_item_path(item), @current_shop),
                    alert: t("vendors.items.product_created_no_variant")
      end
    end

    def handle_failed_creation(item, errors)
      Rails.logger.warn("⚠️ [Employees::ItemsController] Échec création - erreurs: #{errors.join(', ')}")
    ...
    def handle_failed_update(item, errors)
      Rails.logger.warn("⚠️ [Employees::ItemsController] Échec mise à jour - erreurs: #{errors.join(', ')}")
    ...
    def item_params
      params.require(:item).permit(
        ...
        item_attributes_attributes: [
          ...
          { attribute_values_attributes: [ :id, :value, :position, :_destroy ] }
        ],
        variants_attributes: [
