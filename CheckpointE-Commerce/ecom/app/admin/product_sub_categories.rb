ActiveAdmin.register ProductSubCategory do
  menu parent: "Catalogue", priority: 3, label: "Sous-catégories"

  # Specify parameters which should be permitted for assignment
  permit_params :name, :description, :is_active, :product_category_id, :position, :icon

  # or consider:
  #
  # permit_params do
  #   permitted = [:name, :slug, :description, :is_active, :product_category_id, :position]
  ...
  index do
    selectable_column
    id_column
    column "Icône" do |subcategory|
      if subcategory.icon.attached?
        image_tag subcategory.icon, style: "max-width: 50px; max-height: 50px; border-radius: 4px;", alt: subcategory.name
      else
        "—"
      end
    end
  ...
  show do
    ...
    panel "🔗 Lien Public de la Sous-catégorie" do
  ...
  form do |f|
    ...
    f.inputs do
      ...
      f.input :position
      f.input :icon, as: :file, label: "Icône de la sous-catégorie", hint: "Taille recommandée : 400×400px (carré). PNG avec fond transparent de préférence. Affiché en cercle (48px–80px) dans la nav, et en carte pleine largeur (160px) sur les pages boutique. Taille max : 5MB."
    end
    f.actions
  end
end
